...
public interface StaffMapper extends BaseMapper<Staff,String> {
    ...
    @Select("select s.name sName ,s.login_name as loginName ,max(phone) phone ,max(urge_group) ugroup ,c.name cName , ifnull(sum(sum_arrears),0) as arrears  , count(ca.sum_arrears) size from t_staff as s left  join t_company c on s.company_id=c.id  left join t_case ca on( s.login_name = ca.staff_id and ca.status=3) group by login_name ,ca.status")
    List<Map> listStaff();
